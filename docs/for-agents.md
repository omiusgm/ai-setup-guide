...
## Автономный минимум для этого проекта

Если цель — меньше ручных пингов и больше работы по кнопке/расписанию, держи базовый набор так:

- **`ai-content-scan-pipeline`** — triage, queue и digest для AI Radar-потока
...
- **`youtube-content`** — если надо разбирать YouTube и превращать это в заметки/посты
...
# АВТОМАТИЗАЦИЯ: /loop, Remote Tasks, HANDOFF.md

## /loop — агент работает по расписанию

Команда `/loop` запускает агента в цикле: выполнил задачу → подождал → выполнил снова. Работает пока не скажешь стоп.

```
/loop 1h проверяй новые лиды в CRM и пиши бриф в /briefs
/loop 30m мониторь рекламный кабинет, алерт если CTR упал
```

Два режима:
- С интервалом: `/loop 5m`, `/loop 1h` — фиксированная пауза между итерациями
- Динамический: `/loop` без интервала — агент сам выбирает когда проснуться

**Ограничение:** работает пока компьютер включён. Закрыл крышку — агент уснул.

## Remote Tasks — агент на серверах Anthropic

Для задач которые должны работать 24/7 — Remote Tasks. Агент выполняет задачу на серверах Anthropic, ноутбук может быть выключен.

Настройка: claude.ai → Projects → Scheduled Tasks. Без кода.

Примеры:
- Утренний брифинг в 7:00 в Telegram
- Еженедельный отчёт по метрикам
- Ночной аудит безопасности
- Сводка по новым репозиториям, новостям и видео

## HANDOFF.md — память между сессиями

Файл куда агент записывает прогресс. Не создаётся автоматически — нужно попросить:

```
После каждых 10 действий обновляй HANDOFF.md: что сделано, что осталось, какие решения принял
```

Утром открываешь один файл — понимаешь всё что произошло за ночь.

## Экономия токенов при автоматизации

- Для мониторинга используй Haiku вместо Opus — кратно дешевле
- Разбивай ночные задачи на маленькие итерации — при сжатии контекста теряется меньше
- `/loop` с Caveman-промптом экономит ещё больше
